...
# BQ27441 Battery Management System
class BQ27441:
    ...
    def state_of_charge(self):
        try:
            return self._rd_word(0x1C) / 30  # Assuming 3000mAh = 100%
        except:
            return 0
...
class PamirProtocol:
    ...
    def __init__(self):
        ...
        self.uart = machine.UART(
            0, baudrate=115200, tx=machine.Pin(0), rx=machine.Pin(1)
        )
        ...
        self.last_power_report_time = 0
        self.power_report_interval_ms = 30000  # Report every 30 seconds

        # Initialize BQ27441 Battery Management System
        try:
            self.bms = BQ27441(design_capacity_mAh=3000)
            self.bms_available = True
            self.debug_print("BQ27441 Battery Management System initialized")
        except Exception as e:
            self.bms_available = False
            self.debug_print(f"Failed to initialize BQ27441: {str(e)}")
        ...
        self.sam_interrupt.irq(
            trigger=machine.Pin.IRQ_RISING, handler=self._loading_terminator
        )
    ...
    def _process_led_packet(self, packet):
        ...
        self.debug_print(
            f"LED packet: cmd={cmd_type}, id={led_id}, RGB=({r},{g},{b}), time={time_value}"
        )
        ...
        if cmd_type == LED_CMD_QUEUE:
            ...
            if len(self.led_queues[led_id]) > 0:
                self.debug_print(
                    f"Executing sequence for LED {led_id} with {len(self.led_queues[led_id])} steps"
                )
                ...
                self.debug_print(
                    f"Execute command received but queue is empty for LED {led_id}"
                )
    ...
    def _execute_led_sequence(self, led_id):
        ...
        try:
            ...
            self.debug_print(
                f"Starting LED sequence for LED {led_id}, {sequence_length} steps"
            )
    ...
    def _process_power_packet(self, packet):
        ...
        self.debug_print(
            f"Received power packet: cmd=0x{command:02x}, param=0x{param:02x}, data=0x{data1:02x} 0x{data2:02x}"
        )

        if command == POWER_CMD_QUERY:
            # Respond with current power state
            self.debug_print(
                f"Responding to power query with state: {self.power_state}"
            )
            ...
            # Send acknowledgment
            self.send_packet(
                TYPE_POWER | POWER_CMD_SHUTDOWN, data1, 0x01
            )  # ACK with original mode and flag=1
    ...
    def run(self):
        ...
        while True:
            ...
            if (
                utime.ticks_diff(current_time, last_status_time) > 5000
            ):  # Every 5 seconds
                # If no communication received for too long and Linux was booted, handle potential crash
                if (
                    self.linux_booted
                    and not self.shutdown_requested
                    and utime.ticks_diff(current_time, self.last_receive_time) > 30000
                ):  # 30 seconds
                    self.debug_print(
                        "WARNING: No communication from Linux host for 30 seconds"
                    )
                    self.send_debug_code(
                        DEBUG_CAT_POWER, 0xFE, 0x01
                    )  # Host potentially crashed

                last_status_time = current_time

            # Check if it's time to report power metrics
            self.check_power_reporting()

            # Check for simultaneous UP+SELECT press to trigger shutdown
            if self.thread_handoff_complete:
                if (
                    self._get_debounced_state(self.btn_up)
                    and self.btn_select.value() == 1
                ):
                    ...
                    while utime.ticks_diff(utime.ticks_ms(), start_time) < 10000:
                        ...
                        if utime.ticks_diff(utime.ticks_ms(), start_time) >= 2000:
                            ...
                            self.send_packet(
                                TYPE_POWER | POWER_CMD_SHUTDOWN, SHUTDOWN_MODE_NORMAL, 0
                            )
                            # Update state
                            self.power_state = POWER_STATE_OFF
                            self.shutdown_requested = True
    ...
    # Power metrics reporting methods
    def send_power_current(self, current_ma):
        """Send current power draw in mA (16-bit value)"""
        self.debug_print(f"Sending current: {current_ma} mA")
        self.send_packet(
    ...
    def send_power_battery(self, battery_pct):
        """Send battery state of charge in percentage (16-bit value)"""
        self.debug_print(f"Sending battery: {battery_pct}%")
        self.send_packet(
    ...
    def send_power_temperature(self, temp):
        """Send temperature in 0.1°C units (16-bit value)"""
        # Convert to 0.1°C units (e.g., 25.5°C = 255)
        temp_int = int(temp * 10)
        self.debug_print(f"Sending temperature: {temp}°C ({temp_int} decidegrees)")
        self.send_packet(
            TYPE_POWER | POWER_CMD_TEMP,
            temp_int & 0xFF,  # Low byte
            (temp_int >> 8) & 0xFF,
        )  # High byte

    def send_power_voltage(self, voltage_mv):
        """Send voltage in mV (16-bit value)"""
        self.debug_print(f"Sending voltage: {voltage_mv} mV")
        self.send_packet(
    ...
    def read_power_sensors(self):
        """Read values from power-related sensors

        If BQ27441 Battery Management System is available, read from it.
        Otherwise, generate simulated values.

        Returns:
            tuple: (current_ma, battery_pct, temp_c, voltage_mv)
        """
        if self.bms_available:
            try:
                # Real readings from BQ27441
                current_ma = abs(self.bms.avg_current_mA())
                battery_pct = min(100, max(0, int(self.bms.state_of_charge())))
                temp_c = self.bms.temp_C()
                voltage_mv = int(self.bms.voltage_V() * 1000)  # Convert V to mV

                self.debug_print(
                    f"BMS readings: current={current_ma}mA, battery={battery_pct}%, "
                    f"temp={temp_c}°C, voltage={voltage_mv}mV"
                )

                return (current_ma, battery_pct, temp_c, voltage_mv)
            except Exception as e:
                self.debug_print(f"Error reading from BMS: {str(e)}")
                # Fall back to simulated values if there's an error

        # Simulate values if BMS is not available or reading failed
        # Simulate current varying between 200-300 mA
        current_ma = 250 + (utime.ticks_ms() % 100)

        # Simulate battery draining slowly over time
        time_hours = utime.time() / 3600  # Convert seconds to hours
        battery_pct = max(0, 100 - int(time_hours % 100))

        # Simulate temperature between 25-30°C based on time
        temp_c = 25.0 + ((utime.ticks_ms() % 50) / 10)

        # Simulate voltage around 3.7-4.2V based on battery
        voltage_mv = 3700 + int(battery_pct * 5)

        self.debug_print(
            f"Simulated readings: current={current_ma}mA, battery={battery_pct}%, "
            f"temp={temp_c}°C, voltage={voltage_mv}mV"
        )

        return (current_ma, battery_pct, temp_c, voltage_mv)

    def report_power_metrics(self):
        """Report all power metrics"""
        # Only send if Linux has booted and we're not shutting down
        if not self.linux_booted or self.shutdown_requested:
            return

        # Read sensor values
        current_ma, battery_pct, temp_c, voltage_mv = self.read_power_sensors()

        # Send all metrics
        self.send_power_current(current_ma)
        self.send_power_battery(battery_pct)
        self.send_power_temperature(temp_c)
        self.send_power_voltage(voltage_mv)

        # Update last report time
        self.last_power_report_time = utime.ticks_ms()

    def check_power_reporting(self):
        """Check if it's time to report power metrics"""
        current_time = utime.ticks_ms()

        # Report power metrics periodically if Linux has booted
        if (
            self.linux_booted
            and not self.shutdown_requested
            and utime.ticks_diff(current_time, self.last_power_report_time)
            > self.power_report_interval_ms
        ):
            self.report_power_metrics()
